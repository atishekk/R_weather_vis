...
pal_temp <- colorNumeric(palette = c("#6600ff","yellow","#cc0000"), domain = c(10,40))
    ...
    output$dist <- renderPlot({
      switch(input$pol,
             temp = subset(temp_city(),!is.na(Temp)) %>% 
               ggplot(aes(x=Temp,fill=factor(Group,levels=c("Rain","Fog","Cloudy","Clear")))) + 
               geom_histogram(binwidth=5,color="#333333",alpha=0.5) +
               guides(fill=guide_legend(title="Description")) + 
               scale_fill_manual(values=c("Rain"="#0066ff","Fog"="#00ff99","Cloudy"="#ffff00",
